...
This recursive structure (any hypothesis can spawn its own hypothesis set) enables
deep uncertainty modeling in future extensions. For MVP, the scope is simply the
session-level frame that defines what the named hypotheses and H_other are alternatives *to*.
...
#+begin_src text
inputs:
  scope text                    # what the hypotheses are about
  roots: List[RootSpec]         # named hypotheses with exclusion clauses
  credits B
...
- Ledger with H_other absorber
- Template obligations (or simplified template)
- Credit allocation (with beta fraction cap to prevent infinite descent)
...
See future documentation for full recursive MECE specification.
...
- Frontier ordering is deterministic: when multiple hypotheses tie for max p_ledger,
  they're ordered by canonical_id, not by the order they were provided
- Recursion-ready: child nodes can use parent_key:child_id format while maintaining
  canonical ordering within each level
...
This prevents permutation-breaking when the same hypothesis is phrased slightly
differently or when input sources vary in whitespace conventions.
