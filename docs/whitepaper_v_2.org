...
** 12.3 Fixed \(\gamma\) (open-world only)
...
- Then set \(p(H_{\text{other}})=\gamma\) and rescale named \(p\) values proportionally so total sums to 1.
...
** 13.4 Starvation avoidance (alternatives)
...
- evidence_ids used (may be empty)
...
ABDUCTIO maintains:
- Ledger probabilities \(p_{\text{ledger}}(h)\) over roots (MECE bookkeeping).
...
Operational meaning:
- \(p_{\text{ledger}}(h)\) is the engine’s *current, normalized allocation of credence mass* across the MECE root set for the session scope.
...
3. Completeness beyond the declared MECE: in closed-world mode, exhaustiveness is assumed; in open-world mode, \(h_{\text{other}}\) captures residual uncertainty but does not enumerate unknown mechanisms.
...
This appendix is illustrative (non-normative) and shows the *mechanics* of v2: template parity, symmetric log-space updates, and Delta-\(w\) enforcement.

** E.1 Scope and MECE roots (closed world)
- *Scope*: "Primary cause of the 2026-01-04 web service outage (15:02–15:27 UTC) at Company Z."
- *Mode*: CLOSED_WORLD (no \(H_{\text{other}}\)).
- *Named roots* (MECE for this scope):
...
* Appendix F: Worked Example (Open-World Mode with \(H_{\text{other}}\) and \(\gamma\))
This appendix is illustrative (non-normative) and highlights open-world behavior: residual uncertainty can *rise* even when one named hypothesis leads, via the deterministic \(\gamma\) rule.

** F.1 Scope and MECE+Other roots (open world)
...
| H_other | Other/unmodeled explanation. | Not any named root as specified. |
...
Assume (illustrative) that Feasibility has been *scoped* and lightly assessed from domain priors (not paid-for here; if unpaid, they should remain at defaults in a strict engine). To keep this example consistent with the v2 "paid-for moves" principle, we will use defaults for any unpaid slots except those evaluated above.

Thus:
- For \(H1\): Availability \(p=0.75,k=0.55\); Fit \(p=0.60,k=0.35\); Feasibility default; Defeater default.
...
- select the band \(F\) by the v2 rule, and set \(F\leftarrow \mathrm{sort}(F)\), \(i\leftarrow 0\).
...
** Formal Proof of Bounded Decomposition Invariance (ABDUCTIO v2)
:PROPERTIES:
...
This section establishes decomposition invariance properties for ABDUCTIO v2 under *semantically equivalent decompositions** (identical assessed leaves, differing only in binary tree shape). It proves exact invariance for confidence propagation and associative OR aggregation, and proves a *tight global bound** on the non-associativity of the NEC soft-AND operator. The bound is then propagated to evidence weights and ledger probabilities.
